			...
			for tok in sent:
				...
				if tok[2] in ['like', 'want', 'wan'] and tok[3].startswith('v'): 
					...
					#	if subj == 'I' or subj == 'NONE':
						...
						data.append(['emotion', function, tok[2], neg[1], aux, aux_stem, subj, subj_stem, speaker_role, saying, age, len(sent)])
			...
			for tok in sent:
				...
				if tok[2] in ['know', 'think', 'remember'] and tok[3].startswith('v'): 
					...
					#	if subj == 'I' or subj == 'NONE':
						...
						data.append(['theory of mind', function, tok[2], neg[1], aux, aux_stem, subj, subj_stem, speaker_role, saying, age, len(sent)])
				...
				if tok[2] in ['do', 'can']:
					...
					if idx < len(sent) and sent[idx][1] in ['not', 'no', "n't"] and post not in SUBJ:
						...
						if int(neg[0]) == len(sent): 
							...
							data.append(['motor_control', function, head[2], neg[1], aux, aux_stem, subj, subj_stem, speaker_role, saying, age, len(sent)])
				...
				if tok[2] in ['be']: # and tok[1] in ['am', 'was', 'is', 'are', 'were']:
					...
					if neg != '':
						...
						if pred != '' and subj_stem not in ['there', 'There'] and post not in SUBJ:
							...
							data.append(['learning', function, head[2], neg[1], pred_pos, pred_stem, subj, subj_stem, speaker_role, saying, age, len(sent)])
			...
			for tok in sent:
				...
				if tok[2] in ['why', 'Why']: 
					...
					#	if subj == 'I' or subj == 'NONE':
						...
						data.append(['unknown', function, tok[2], neg[1], aux, aux_stem, subj, subj_stem, speaker_role, saying, age, len(sent)])
			...
			for tok in sent:
				...
				if tok[3].startswith('v') and tok[2] not in ['like', 'want', 'know', 'think', 'remember', 'have']: 
					...
					#	if subj == 'I' or subj == 'NONE':
						...
						data.append(['perception', function, tok[2], neg[1], aux, aux_stem, subj, subj_stem, speaker_role, saying, age, len(sent)])
