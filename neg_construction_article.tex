...
The current study builds on previous research and addresses the methodological issues discussed in the previous section in two ways. First, this study uses the largest available corpus of child-parent interactions that aggregates speech samples from X children between the ages of 1-6 years. If the lack of consensus in previous research was mainly due to the small number of investigated children, increasing the number of children we study should minimize this issue. Using a larger corpus will also provide denser samples at each period of children's development overall.

Second in annotating and classifying negative utterances, we focus on the syntactic constructions that negative morphemes participate in. While it is difficult to manually annotate for the communicative functions of negation in large corpora, it is relatively easier to automatically detect specific negative constructions. These constructions in turn tend to convey

This approach is reminiscent of Choi (1988) who annotated for both communicative functions and their common constructions (Table \ref{tab:summary}).

Important to point out that in using large number of children and a larger corpus, we hoped to capture large scale stages that hold on average across children despite individual variability.

By focusing on negative constructions produced by children \ldots{} previous studies have mainly focused on early minimal forms of negation and manually annotated their communicative functions in context. This approach relies on contextual information and the annotators' understanding of the communicative function of negation. Studies vary on which basic categories should be used and
...
One downside to focusing on negative constructions is that we may underestimate children's knowledge of negation. Due to early limited productive capacities, children produce shorter forms before longer ones. Therefore, they can convey a communicative function like rejection using a simple \emph{no} before they produce the full construction ``\emph{I don't want that}.'' Focusing on children's early constructions can give us a late or upper bound estimate on when children can convey a communicative function, but not an early or lower bound.

To make up for this shortcoming, we also investigate children and parents' use of \emph{no} as a response particle.

at the discourse level and capture constructions that it negates in conversation. This measure can potentially capture communicative functions that the response particle \emph{no} captures before the full syntactic forms and constructions are produced by children.

\hypertarget{negative-constructions-and-communicative-functions}{%
\subsection{Negative Constructions and Communicative Functions}\label{negative-constructions-and-communicative-functions}}

Table \ref{tab:constructions},
...
Labeling & NP/ADJ predicates & \textit{that's not a crocodile}; \textit{it's no interesting} & \textit{that's a table}; \textit{it's cool} \\
