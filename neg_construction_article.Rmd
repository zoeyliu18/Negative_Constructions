...
At the discourse level, on the other hand, we aim to analyze utterances where the negative morphemes are discourse response variables instead. To do that and in order to alleviate potential parsing errors, instances starting with the negative morphemes that are discourse response variables (again with a dependency relation *discourse*) were included. In this case, utterances such as "no no I like it" were selected. Then for each negative utterance identified this way, we extracted its previous utterances in the discourse context based on the utterance order and transcript id provided by childes-db. In particular, for child speech, we included interactions (contextual utterances + current negative utterance) where the contextual utterances were produced by either parents or children themselves; and for parent speech, we only included interactions where the contextual utterances were produced by children. Then for every function, we applied the same syntactic features for both negative and positive utterances at the syntactic level to the contextual utterance within each discourse interaction. Therefore in what follows, when describing whether a construction is negative or positive at the discourse leve, we refer to the contextual utterance.


<!--
At the syntactic level, we first identified negative structures which are ones containing the morphemes *no*, *not* and *n't*. Cases consisted of a single or repetitions of negative morphemes (e.g. "no no no") were not included. Instances where the negative morpheme(s) all serve as discourse response variables (has a dependency relation of *discourse* with its syntactic head) were excluded (e.g. "no no I like it"). By contrast, cases with more than one negative morphemes, such as "no I do not want this", were kept as long as at least one of the negative morphemes is not a discourse response variable.-->


 <!--To facilitate identification of negative constructions, we also utilized the available part-of-speech (POS) information initially provided by CHILDES [@sagae2010morphosyntactic] when necessary.-->
...
On the discourse level, we applied the same syntactic features for negative constructions (e.g. ) and their positive counterparts (e.g. ) to the contextual utterance within an interaction (contextual utterance + current negative utterance). This resulted in a total of `r format(sum(subset(discourse_emotion_data,Which=='ratio' & Polarity == 'negative')$Freq), big.mark=",")` negative utterances (child: `r format(sum(subset(discourse_emotion_data_child,Which=='ratio' & Polarity == 'negative')$Freq), big.mark=",")`; parent: `r format(sum(subset(discourse_emotion_data_parent,Which=='ratio' & Polarity == 'negative')$Freq), big.mark=",")`), and a total of `r format(sum(subset(discourse_emotion_data,Which=='ratio' & Polarity == 'positive')$Freq), big.mark=",")` negative utterances (child: `r format(sum(subset(discourse_emotion_data_child,Which=='ratio' & Polarity == 'positive')$Freq), big.mark=",")`; parent: `r format(sum(subset(discourse_emotion_data_parent,Which=='ratio' & Polarity == 'positive')$Freq), big.mark=",")`).
...
&nbsp;
(6) Parent: *I don't want you to do that*; Child: *no I want to*

&nbsp;
(7) Child: *I don't want that*; Parent: *no honey you have to try it*



```{r emotiondiscourse, fig.env="figure", fig.pos = "H", fig.align = "center", fig.width=7, fig.height=5, set.cap.width=T, num.cols.cap=1, fig.cap='Discourse-level Rejection',warning=FALSE}
...
discourse_perception_data <- rbind(discourse_child_existence_neg, discourse_child_existence_pos, discourse_parent_existence_neg, discourse_parent_existence_pos, discourse_child_possession_neg, discourse_child_possession_pos, discourse_parent_possession_neg)
...
children <- as.vector(unique(subset(possession, Age<=36)$Child))
...
```{r existencediscourse, fig.env="figure", fig.pos = "H", fig.align = "center", fig.width=7, fig.height=5, set.cap.width=T, num.cols.cap=1, fig.cap='Discourse-level Non-existence',warning=FALSE}
...
```{r prohibitiondiscourse, fig.env="figure", fig.pos = "H", fig.align = "center", fig.width=7, fig.height=5, set.cap.width=T, num.cols.cap=1, fig.cap='Discourse-level Prohibition',warning=FALSE}
...
```{r inabilitydiscourse, fig.env="figure", fig.pos = "H", fig.align = "center", fig.width=7, fig.height=5, set.cap.width=T, num.cols.cap=1, fig.cap='Discourse-level Inability',warning=FALSE}
...
```{r learningdiscourse, fig.env="figure", fig.pos = "H", fig.align = "center", fig.width=7, fig.height=5, set.cap.width=T, num.cols.cap=1, fig.cap='Discourse-level Labeling',warning=FALSE}
...
```{r epistemicdiscourse, fig.env="figure", fig.pos = "H", fig.align = "center", fig.width=7, fig.height=5, set.cap.width=T, num.cols.cap=1, fig.cap='Discourse-level Epistemic Negation',warning=FALSE}
...
```{r possessiondiscourse, fig.env="figure", fig.pos = "H", fig.align = "center", fig.width=7, fig.height=5, set.cap.width=T, num.cols.cap=1, fig.cap='Discourse-level Possession',warning=FALSE}
