			...
			for tok in sent:
				...
				if tok[2] in ['like', 'want', 'wan'] and tok[3].startswith('v'): 
					...
					#	if subj == 'I' or subj == 'NONE':
						...
						data.append(['emotion', function, tok[2], pos[1], aux, aux_stem, subj, subj_stem, speaker_role, saying, age])
			...
			for tok in sent:
				...
				if tok[2] in ['know', 'think', 'remember'] and tok[3].startswith('v'): 
					...
					#	if subj == 'I' or subj == 'NONE':
						...
						data.append(['theory of mind', function, tok[2], pos[1], aux, aux_stem, subj, subj_stem, speaker_role, saying, age])
			...
			for tok in sent:
				if tok[2] in ['do', 'can'] and tok[3] in ['mod']:
					...
					if idx == '':  ### e.g. I can ###
						...
						data.append(['motor_control', function, head[2], 'NONE', aux, aux_stem, subj, subj_stem, speaker_role, saying, age])
					...
					if idx != '' and idx < len(sent) and sent[idx][1] not in ['not', 'no', "n't"] and post not in SUBJ:
						...
						if int(pos[0]) == len(sent): 
							...
							data.append(['motor_control', function, head[2], pos[1], aux, aux_stem, subj, subj_stem, speaker_role, saying, age])
				...
				if tok[2] in ['be']: # and tok[1] in ['am', 'was', 'is', 'are', 'were']:
					...
					if pos != '':
						...
						if pred != '' and subj_stem not in ['there', 'There'] and post not in SUBJ:
							...
							data.append(['learning', function, head[2], pos[1], pred_pos, pred_stem, subj, subj_stem, speaker_role, saying, age])
