...
In this study, we provide evidence that suggests the functional development of negation in stages cannot be taken for granted. Given previous studies and our own, we conclude that it is possible that negation starts as an abstract concept that can be used to perform different communicative functions, and it is possible that it develops relatively quickly in functional stages becoming more abstract between 18-30 months of age. 
...
@darwin1872expression proposed one of the earliest accounts that explained the emergence of negation based on its communicative function. He hypothesized that negation originates from expressions of human emotions and desires. He considered nodding and shaking as the earliest expressions of affirmation and negation respectively. Darwin argued that: "With infants, the first act of denial consists in refusing food; and I repeatedly noticed with my own infants, that they did so by withdrawing their heads laterally from the breast, or from anything offered them in a spoon. In accepting food and taking it into their mouths, they incline their heads forwards ... [moreover] ... when the voice is exerted with closed teeth or lips, it produces the sound of the letter *n* or *m*. Hence we may account for the use of the particle *ne* to signify negation, and possibly also of the Greek *µn* in the same sense." In later research, this communicative function of negation was referred to as "rejection" or "refusal". Darwin did not explain how negation comes to express communicative functions other than rejection.

Much later, @mcneill1968 proposed a five-stage account for the conceptual development of negation. They studied the development of three Japanese negative morphemes (Nai, Iya, Iiya) in the speech of a 27-month-old Japanese speaking girl called Izanami. According to @mcneill1968, *nai* expresses falsity of statements (e.g. "*no* [that's not an apple]), *Iya* expresses desires (e.g. "*no* [I don't want an apple]"), and *Iiya* expresses contrast (e.g. *no* [I didn't have an apple. I had a pear]). In the first stage, Izanami used a simple negation like *nai* to express non-existence of events and objects. The study also reported the early use of *shira-nai* (I don't know) but did not incorporate it into its theoretical account. In the second stage, Izanami used negation to mark incorrectness of statements - similar to saying "false". Such uses of negation were labeled as "denials" in later research. In stage three, negation was also used to express disapproval or rejection - similar to saying "I don't want that". In the fourth stage, Izanami used negation to express contrasts - similar to saying "not this but something else". Finally in the last stage, Izanami had an abstract concept of negation. According to @mcneill1968, these stages took about five months and contrary to Darwin's account, it started with expressing external states (non-existence of objects) before internal ones (rejection). 

@bloom1970language considered three communicative functions for early negation: non-existence, rejection, and denial. She studied three children, two from 19 months and another from 21 months of age. She argued that in all three children, negation followed three phases. In the first phase, negation was mostly used to express non-existence. In the second phase, negation was used to convey rejection as well. And finally in phase three, examples of denial negation were also attested. Table \@ref(tab:bloom) provides a few examples for each category. Many of these examples do not immediately stand out as instances of their category outside the context of use. This is partly because many early examples are minimal combinations of negation and other words and rely heavily on the context of use for their interpretations. It is therefore hard to assess the intention behind the use of negation in such cases.
...
@cameron2007part focused on dense recordings of a single English speaking child between 27 to 39 months. They classified his utterances into seven communicative functions, mostly adopted from @choi1988semantic and dropping the categories of "normative" and "inferential" negation. They found examples of all these functions in Brian's early speech. Starting at 2;3, single-word discourse-level *no* was used to convey most functions but gradually other forms using *not*, *don't*, *can't*, or *won't* emerged and replaced *no* in usage. For example with inability and prohibition, Brian mostly used *no* and *not* at 2;3 but switched to *can't* to express inability, and *don't* to express prohibition almost exclusively at 3;3. @cameron2007part argued that at 2;3 Brian had a broad conceptualization of negation and likely represented it as a "unitary category in conceptual space".

In a recent study, @nordmeyer2018individual looked at the speech of five children between the 12-36 months of age (1-3 years) in the Providence corpus [@demuth2006word] and classified children's negative utterances into seven functional categories: "disappearance", "prohibition", "self-prohibition", "rejection" (refusal), "failure", "denial", and "unfulfilled expectations". Self-prohibition referred to cases where children addressed a prohibition to themselves (e.g. saying "no" to themselves when reaching for a forbidden object) and "unfulfilled expectations" referred to cases that expressed surprise when an object was not in an expected place, similar to some cases of non-existence in previous research. They found that refusals (rejections) and denials are the most common functions in children's productions and that children in their sample vary on which function is produced first. In line with @de1979form, they concluded that the developmental trajectory of different communicative functions of negation may not be as consistent across individuals as some previous research had suggested.

|Study|Number of Children|Age Range (Months)|Proposed Functional Stages|
|-----|------|--------|--------------------------|
|@mcneill1968|1|27-32 Months|non-existence > rejection > denial (non-contrastive) > rejection > denial (contrastive) |
...
Table \@ref(tab:summary) provides a summary of previous research on the communicative functions of negation in children's speech. As the table shows, there is currently no consensus regarding which functions should be included or in what order they are produced. There may be several reasons why and we are going to discuss four of them. These range from more methodological issues to more theoretical ones. 

First, @de1979form and @nordmeyer2018individual have emphasized that there is considerable variability among children in this respect. Therefore, studies with small numbers of children can reasonably reach different conclusions that are true of their sample but not of the population of, for example English-speaking, children generally. Given that previous studies have used very small sample sizes, they may have not had the power to detect general developmental stages that hold across children on average. Such patterns may be detectable in child language corpora with samples from large numbers of children. 

Second, previous research makes it clear that defining and detecting the communicative functions of negation is not a trivial task. Different studies have sometimes used different basic categories and different criteria for classifying negative utterances. Therefore, what counts as an instance of rejection or non-existence may vary among studies and contribute to the variable findings. Most importantly, previous studies rely on under-specified utterances such as "*no car*" or "*no more*" which are highly ambiguous and can count as an instance of many communicative functions. Researchers often have to rely on the context to utterance in such cases, but this is a difficult task given that context is not fully represented in many child language corpora. More importantly, this approach is not scalable as manual annotations take considerable amount of time and energy. 

Finally, it is possible that there are stages but these stages happen so quickly that they are not detectable by current samples of monthly or fortnightly recordings of children's speech. 
...
Here we build on previous research in two ways. First, we use the largest available corpus of child-parent interactions and aggregate speech samples across X children between the ages of 1-6 years. If variable findings in previous research was mainly due to the small number of investigated children, our study would be able to avoid it. Second in annotating and classifying negative utterances, we focus on the syntactic constructions negative morphemes participate in. While it is difficult to manually annotate for the communicative function of negation especially in large corpora, it is relatively easier to automatically detect what negative morphemes combine with. These combinations and constructions in turn tend to convey 

We see early indications of this approach in 

previous studies have mainly focused on early minimal forms of negation and manually annotated their communicative functions in context. This approach relies on contextual information and the annotators' understanding of the communicative function of negation. Studies vary on which basic categories should be used and 
Here we take a different approach. We focus on larger syntactic constructions that tend to convey particular communicative function. Our emphasis is on the combinatorial capacity of negation. What other functional morphemes or syntactic categories does negation combine with in children's early productions?

We also investigate *no* as a response particle at the discourse level and try to capture constructions that it negates in conversation. This measure can potentially capture communicative functions that the response particle *no* captures before the full syntactic forms and constructions are produced by children. 

## Negative Constructions and Communicative Functions

Similar to @choi1988semantic's approach, we look for common forms and constructions that express negative communicative functions. Table \@ref(tab:constructions), 
